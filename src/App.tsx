...
import CropView from './crop/CropView';
...
export default function App() {
  ...
  const [view, setView] = useState<'practice' | 'crop'>('practice');
  const cropData = useCropStore();
  const pendingCount = Object.keys(cropData.pending).length;
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark"><Volume2 size={19}/></div><div><strong>声线练习室</strong><span>Pronounce / practice</span></div></div><div className="side-label">我的练习</div><nav><button className={view === 'practice' ? 'side-link active' : 'side-link'} onClick={() => setView('practice')}><Mic size={17}/>练习库 <b>{phrases.length}</b></button><button className={view === 'crop' ? 'side-link active' : 'side-link'} onClick={() => setView('crop')}><Scissors size={17}/>静音裁切 {pendingCount > 0 && <b className="side-badge">{pendingCount}</b>}</button><button className="side-link"><Clock3 size={17}/>练习记录</button><button className="side-link"><Check size={17}/>已掌握 <b>{phrases.filter(p => p.status === 'mastered').length}</b></button></nav><div className="sidebar-foot"><div className="streak"><span>连续练习</span><strong>5 <small>天</small></strong><i>↗ +2</i></div><div className="profile"><div className="avatar">YL</div><div><strong>Yuki Lin</strong><span>普通计划</span></div><ChevronRight size={16}/></div></div></aside>
    {view === 'crop' ? (
      <main className="main"><CropView/></main>
    ) : (

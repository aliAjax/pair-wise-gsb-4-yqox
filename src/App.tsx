...
import { Check, ChevronRight, Clock3, Mic, Pause, Play, Plus, RotateCcw, Scissors, Search, Trash2, Volume2 } from 'lucide-react';
import TrimPanel from './trim/TrimPanel';
import { createRecording, loadTrimStore, saveTrimStore } from './trim/storage';
import type { TrimStore } from './trim/types';
...
export default function App() {
  ...
  const [trimStore, setTrimStore] = useState<TrimStore>(() => loadTrimStore());
  ...
  useEffect(() => { saveTrimStore(trimStore); }, [trimStore]);
  useEffect(() => () => window.clearInterval(timer.current), []);
  const startRecord = () => { if (recording) { const recordedSeconds = seconds; setRecording(false); window.clearInterval(timer.current); setRecorded(true); setPhrases(ps => ps.map(p => p.id === selected ? {...p, attempts: p.attempts + 1, status: 'practice', last: '刚刚'} : p)); if (current && recordedSeconds > 0) { setTrimStore(store => createRecording(store, { phraseId: current.id, phraseText: current.text, originalDuration: recordedSeconds }).store); } return; } setSeconds(0); setRecording(true); timer.current = window.setInterval(() => setSeconds(s => s + 1), 1000); };
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark"><Volume2 size={19}/></div><div><strong>声线练习室</strong><span>Pronounce / practice</span></div></div><div className="side-label">我的练习</div><nav><button className="side-link active"><Mic size={17}/>练习库 <b>{phrases.length}</b></button><button className={showTrim ? 'side-link trim-active' : 'side-link'} onClick={() => setShowTrim(v => !v)}><Scissors size={17}/>静音裁切 <b>{trimStore.drafts.length || ''}</b></button><button className="side-link"><Clock3 size={17}/>练习记录</button><button className="side-link"><Check size={17}/>已掌握 <b>{phrases.filter(p => p.status === 'mastered').length}</b></button></nav><div className="sidebar-foot"><div className="streak"><span>连续练习</span><strong>5 <small>天</small></strong><i>↗ +2</i></div><div className="profile"><div className="avatar">YL</div><div><strong>Yuki Lin</strong><span>普通计划</span></div><ChevronRight size={16}/></div></div></aside>
    ...
    </main>{showTrim && <TrimPanel store={trimStore} onChange={setTrimStore} onClose={() => setShowTrim(false)} />}{showAdd && <div className="modal-backdrop" onClick={() => setShowAdd(false)}><div className="modal" onClick={e => e.stopPropagation()}><div className="modal-head"><h2>添加练习句子</h2><button className="icon-btn" onClick={() => setShowAdd(false)}>×</button></div><label>英文句子<textarea autoFocus value={newText} onChange={e => setNewText(e.target.value)} placeholder="例如：I can make this happen."/></label><div className="modal-actions"><button className="secondary" onClick={() => setShowAdd(false)}>取消</button><button className="primary" onClick={addPhrase}>加入句子库</button></div></div></div>}
  </div>;
}
